...
function Categories(props) {
  ...
  return (
    <div className={`flex ${!props.isSidebarOpen?"sidebar":""}  items-center`} style={{transition:"all .5s ease"}}>
      <div className="category relative w-full h-[100vh] bg-[#084c3b] z-10 shadow-lg">
        ...
        <div className="wrapper my-1">
            <div className="searchBar md:translate-x-4 md:-translate-y-1 sm:w-40 md:w-48">
        ...
        <ul className="space-y-3 flex mt-10 flex-col justify-center text-start font-semibold  text-white mx-3 text-sm">
    ...
    <Link to="/about">  <li  className='flex items-center space-x-3 rounded-xl h-10 w-40 px-4 text-start cursor-pointer'><InfoOutlinedIcon fontSize='medium'/><p className=''>About</p></li></Link>    
          <li  className={`flex items-center space-x-3 rounded-xl h-10 w-44 px-4 text-start cursor-pointer ${selectedCategory==='All'?'bg-green-950':''}`}  onClick={()=>{categoryClick("All")}}><CategoryOutlinedIcon fontSize='medium' className={`${selectedCategory==='All'?'text-green-700':''}`}/><p className=''>All</p></li>
          <li  className={`flex items-center space-x-3 rounded-xl h-10 w-44 px-4 text-start cursor-pointer bg ${selectedCategory==='General'?'bg-green-950':''}`} onClick={()=>{categoryClick("General")}} ><TextSnippetOutlinedIcon fontSize='medium' className={`${selectedCategory==='General'?'text-green-700':''}`}/><p className=''>General</p></li>
          <li  className={`flex items-center space-x-3 rounded-xl h-10 w-44 px-4 text-start cursor-pointer ${selectedCategory==='Personal'?'bg-green-950':''}`}  onClick={()=>{categoryClick("Personal")}}><FolderSharedOutlinedIcon fontSize='medium' className={`${selectedCategory==='Personal'?'text-green-700':''}`}/><p className=''>Personal</p></li>
          <li  className={`flex items-center space-x-3 rounded-xl h-10 w-44 px-4 text-start cursor-pointer ${selectedCategory==='Student'?'bg-green-950':''}`}  onClick={()=>{categoryClick("Student")}}><SchoolOutlinedIcon fontSize='medium' className={`${selectedCategory==='Student'?'text-green-700':''}`}/><p className=''>Students</p></li>
          <p></p>
          <p></p>
          <p></p>
<li className="flex items-center space-x-3 rounded-xl h-10 w-44 px-4 translate-x-5 translate-y-4 cursor-pointer "> {!localStorage.getItem('token')?(<div><Link className=" mx-16" to="/login">Login</Link>
