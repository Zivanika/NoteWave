...
const Noteitem = (props) => {
  ...
  <div className="relative h-52 flex flex-col justify-center items-start article">
    ...
    <div className='ml-10'>{formattedDate}</div>
  ...
  <div className='mr-4' style={{fontSize:"12px"}}>{formattedDate}</div>
  <i className="fa-solid fa-pen-to-square mr-4 cursor-pointer" onClick={(e)=>{updateNote(note)}}></i>
  <i className="fa-solid fa-trash mr-4 cursor-pointer" onClick={()=>{deleteNote(note._id)}}></i>
</div>
